...
@section('content')
    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Создать Атрибут</div>
                <div class="card-body">

                    <form method="POST" action="{{ route('admin.adverts.category.attribute.store', $category) }}">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="sort" name="sort" value="{{old('sort')}}" class="form-control{{$errors->has('sort')?' is-invalid':''}}" />
                            @if($errors->has('sort'))
                        ...
                        <div class="form-group">
                            ...
                            <select name="type" id="type"  class="form-control{{$errors->has('type')?' is-invalid':''}}">
                                @foreach($types as $type => $label)
                                    <option value="{{$type}}"  @if($type == old('type')) selected @endif  >{{$label}}</option>
                                @endforeach
                            </select>
                            @if($errors->has('type'))
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="variants" name="variants" class="form-control{{$errors->has('sort')?' is-invalid':''}}" >{{old('variants')}}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <label>
                                <input type="checkbox" name="required" id="required" {{old('required')?'checked':''}} />
                                Обязательное</label>
